...
def get_email_config():
    ...
    password = os.getenv('REPORT_SENDER_PASSWORD', 'WINS@1313')
    receiver = os.getenv('REPORT_RECEIVER_EMAIL', 'keyafusion@gmail.com')
    schedule_time = os.getenv('REPORT_SCHEDULE_TIME', '09:05')
    return {
        'sender': sender,
        'password': password,
        'receiver': receiver,
...
def generate_html_email_body(summary, records):
    """
                    ...
                    <table width="680" cellpadding="0" cellspacing="0" style="background-color:#ffffff; border-radius:12px; overflow:hidden; box-shadow:0 4px 6px -1px rgba(0,0,0,0.05), 0 2px 4px -2px rgba(0,0,0,0.05); border:1px solid #e2e8f0;">
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <p style="color:#94a3b8; margin:0; font-size:14px;">
                                                Daily Attendance Summary Report &bull; {formatted_date}
                                            </p>
                                        </td>
                                        <td align="right">
                        ...
                        <tr>
                            <td style="background-color:#f1f5f9; padding:18px 32px; text-align:center; border-top:1px solid #e2e8f0; font-size:12px; color:#64748b;">
                                ...
                                <p style="margin:4px 0 0 0; color:#94a3b8;">Scheduled Daily Dispatch at {get_email_config()['schedule_time']} AM</p>
                            </td>
                        </tr>
...
def send_daily_attendance_email(target_date=None, to_email=None):
    """
    ...
    print(f"[{datetime.datetime.now()}] Preparing daily attendance email for date {target_date}...")
    
    try:
        summary, records = get_daily_attendance_data(target_date)
        html_content = generate_html_email_body(summary, records)
    ...
    subject = f"Keya Fusion Daily Attendance Report - {target_date} ({summary['present_count']}/{summary['total_employees']} Present)"
    msg['Subject'] = subject
